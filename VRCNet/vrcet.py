...
from VRCNet.model_utils import *
...
class Model(nn.Module):
    def __init__(self, size_z=128, global_feature_size=1024):
        ...
        layers = [int(i) for i in [1,1,1,1]]
        knn_list = [int(i) for i in [16]]
        ...
        self.decoder = MSAP_SKN_decoder(num_fps=2048, num_fine=2048, num_coarse=2048,
                                        num_coarse_raw=1024, layers=layers, knn_list=knn_list,
                                        pk=10, local_folding=True, points_label=True)
...
if __name__ == "__main__":
    ...
    checkpoint = torch.load("checkpoint/pretrained_vrcnet_2048.pth")
    ae_pal = ae
    print(ae_pal.load_state_dict(checkpoint['net_state_dict'], strict=False))