...
from ASFMNet.pcn import Encoder, Decoder
from ASFMNet.modelutils import *
...
class ASFM(nn.Module):
    def __init__(self, step_ratio=4, rt_coarse=False,c3d=False):
    ...
    def forward(self, x):
        '''
         ...
         y_fine(bs, 4096, 3)
        '''
        v = self.encoder(x.permute(0, 2, 1).contiguous())
        ...
        inputs_fps = symmetric_sample(
            x.contiguous(), 512 // 2)  # (bs, 512, 3)
        # print("inputs_fps: ", y_coarse.shape)

        y_fine_1024 = torch.cat([coarse_fps, inputs_fps], dim=1)  # (bs, 1024, 3)

        y_fine = self.refiner(v, y_fine_1024.permute(
            0, 2, 1).contiguous(), self.step_ratio)  # (bs, 4096, 3)

        # print("y_fine: ", y_fine.shape)
        if self.c3d:
            return fps_subsample(y_fine,2048)

        if self.rt_coarse:
            return fps_subsample(coarse_fps,256), y_fine
        else:
            return y_fine
